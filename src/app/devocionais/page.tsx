    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50">
      ...
      <main className="py-6 sm:py-8 px-3 sm:px-4">
        <div className="container max-w-7xl mx-auto">
          ...
          {/* Enhanced Stats Cards - Moved to end of page */}
          <motion.div
            initial={{ opacity: 0, y: 30 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      {devotionals.slice(0, 9).map((devotional) => (
                        <motion.div
                          ...
                          className={`relative p-4 rounded-lg border-2 transition-all duration-200 cursor-pointer ${selectedDevotionals.includes(devotional.id)
                        ...
                        >
                          {/* Checkbox visual */}
                          <div className={`absolute top-2 right-2 w-5 h-5 rounded-full border-2 flex items-center justify-center ${selectedDevotionals.includes(devotional.id)
            ...
            <motion.div whileHover={{ scale: 1.05, y: -5 }} transition={{ type: "spring", stiffness: 300 }}>
              <Link href="/devocionais" className="block cursor-pointer" onClick={() => setSelectedTag(null)}>
                <div className="bg-white/80 backdrop-blur-sm rounded-xl sm:rounded-2xl p-4 sm:p-6 lg:p-8 border border-white/20 text-center shadow-xl hover:shadow-2xl transition-all duration-300 relative overflow-hidden hover:border-emerald-300 hover:bg-white/90">
                  ...
                  <div className="relative z-10">
                    ...
                    <div className="text-xs text-emerald-500 mt-2 font-medium">Limpar Filtros</div>
                  </div>
                </div>
              </Link>
            </motion.div>
